...
const logState = ctx => next => (...args) => {
  ...
  if (!args[0].back) {
    ...
    localStorage['refast-dodoList-log'] = JSON.stringify(refast_todoList_log);
  }
  next(...args);
}
// logState 可以是一个函数 
...
class TodoList extends Component {
  ...
  componentDidMount() {
    localStorage['refast-dodoList-log'] = JSON.stringify([[]]);
    ...
    return (
      <div className = 'todoList'> 
         ...
         <div style={{width:'100%', float:'left', marginTop:30}}>
           一共 {refast_todoList_log && refast_todoList_log.length}步,
           当前第 {step}步
           <button onClick={
             () => {
               if (step >= 2) {
